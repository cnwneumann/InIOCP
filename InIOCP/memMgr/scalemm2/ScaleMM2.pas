...
{
Fast Scaling Memory Manager 2.8 for Delphi
...
Change log:
 ...
 - rare AV fixed in TGlobalMemManager.GetNewThreadManager (thanks to Moln醨 Attila)
 ...
 Version 2.7 (29-11-2016) 
   (many thanks to Moln醨 Attila for testing, finding and solving these bugs!)
...
var
  ...
  ot: PBaseSizeManager;
begin
  //simple but very fast busy flag/soft lock
  if Boolean(FBusyLock) then
    FastBusyLock
  else
    FBusyLock := 1;
...
begin
  while FBusyLock > 0 do
    Sleep(1);
  FBusyLock := 1;

  //inc(FBusyLock);
//  asm
//    lock inc dword ptr [eax+$14];
//    jnz BusyLock;
//  end;

  {
  if AtomicIncrement(FBusyLock) > 1 then
  begin
    AtomicDecrement(FBusyLock);
...
function TThreadMemManager.IsBusyLocked: boolean;
begin
  Result := (FBusyLock > 0);
end;
