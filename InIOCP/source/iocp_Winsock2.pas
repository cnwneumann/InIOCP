...
type
  // 有的 Delphi 7 版本的 windows.pas 单元缺少以下定义，请自己增加：
  // UINT_PTR = Cardinal;
  // ULONG_PTR = Cardinal;
  // DWORD_PTR = ULONG_PTR;
  TSocket = UINT_PTR;
...
type
 ...
 {$ELSE}
  ...
  MarshaledAStringList = ^PAnsiChar;
 {$IFEND}
...
function WSAConnectByNameW(s: TSocket; nodename, servicename: LPWSTR; var LocalAddressLength: DWORD;
  ...
  const timeout: timeval; Reserved: LPWSAOVERLAPPED): BOOL; {$IF CompilerVersion >= 18} inline; {$IFEND} overload;
function WSAConnectByNameW(s: TSocket; nodename, servicename: LPWSTR; var LocalAddressLength: DWORD;
  var LocalAddress: TSockAddr; var RemoteAddressLength: DWORD; var RemoteAddress: TSockAddr;
